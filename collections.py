...
# NOTE: Dealing with the impedance mismatch between Q symbols, chars and
# char lists and Python strings is a major pain-point
CHAR_CODE = 10
...
class q_list(list):
    ...
    def _write_self(self, message):
        message.fromstring(parser.write_byte(0)
                           + parser.write_integer(len(self)))
        for el in self:
            if self.type.code != 0:
...
class table(object):
    ...
    def __init__(self, val, keys=[]):
        if isinstance(val, flip):
            ...
            self._data._values = table(
                izip(non_keys, [val[x] for x in non_keys]))
            self._type = q_dict
        else:
            self._data = flip(val)
            self._type = flip
...
parser.types['dict'] = TranslateType(
    q_dict, 99, overwrite_write=q_dict._write, overwrite_read=q_dict._read)
parser.types['flip'] = TranslateType(
    flip, 98, overwrite_write=flip._write, overwrite_read=flip._read)
parser.types['list'] = TranslateType(
    q_list, 0, overwrite_write=q_list._write, overwrite_read=q_list._read)
