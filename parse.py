...
class Parser:
    ...
    def __init__(self, types):
        self.types = types
        # Defines order of type code lookup (important for types like q_str
        # that are sub_types of a type already used for translation)
        self.type_order = [
            'char', 'symbol', 'month', 'date', 'minute', 'second', 'time', 'datetime',
            'int', 'long', 'float', 'bool', 'real', 'short', 'byte', 'dict', 'flip', 'table']
    ...
    def update_types(self):
        ...
        self._type_dict = dict(
            map(lambda x: (x.type, x), self.types.itervalues()))
        self._type_preferences = [(x.type, x)
                                  for x in map(self.types.get, self.type_order)]
        self.code_dict = dict(
            map(lambda x: (x.list_code, x), self.types.itervalues()))

    def type(self, x):
        if isinstance(x, q_none):
            return self.code_dict[x.code]
        if isinstance(x, list) or (isinstance(x, q_str) and not x.is_char):
        ...
        for (inherited_type, t) in self._type_preferences:
            if isinstance(x, inherited_type):
                return t
