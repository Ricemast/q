...
shandlers['int'] = specials(
    2147483647, 2147483647, -2147483647, -2147483647, -2147483648, q_none(6))
shandlers['long'] = specials(9223372036854775807, 9223372036854775807, -
                             9223372036854775807, -9223372036854775807, -9223372036854775808, q_none(7))
shandlers['date'] = specials(
    2147483647, date.max, -2147483647, date.min, -2147483648, q_none(14))
shandlers['month'] = specials(2147483647, month.from_date(
    date.max), -2147483647, month.from_date(date.min), -2147483648, q_none(13))
shandlers['datetime'] = specials(
    float('inf'), datetime.max, float('-inf'), datetime.min, float('nan'), q_none(15))
shandlers['time'] = specials(
    2147483647, time.max, -2147483647, time.min, -2147483648, q_none(19))
shandlers['minute'] = specials(2147483647, minute.from_time(
    time.max), -2147483647, minute.from_time(time.min), -2147483648, q_none(17))
shandlers['second'] = specials(2147483647, second.from_time(
    time.max), -2147483647, second.from_time(time.min), -2147483648, q_none(18))
...
class TranslateType(object):
    ...
    def read_data(self, endianness, offset, bytes):
        if self.overwrite_read:
            ...
            val = struct.unpack(endianness(self.format),
                                bytes[offset:offset + self.offset])[0]
...
types['int'] = TranslateType(int, 6, 'i', 4, additional_read=shandlers['int'].read(
    lambda x: x), additional_write=shandlers['int'].write(lambda x: x))
types['short'] = TranslateType(code=5, format='h', offset=2, additional_read=shandlers[
                               'short'].read(lambda x: x), additional_write=shandlers['short'].write(lambda x: x))
types['float'] = TranslateType(float, 9, 'd', 8)
types['real'] = TranslateType(code=8, format='f', offset=4)
types['long'] = TranslateType(long, 7, 'q', 8, additional_read=shandlers['long'].read(
    lambda x: x), additional_write=shandlers['long'].write(lambda x: x))
types['char'] = TranslateType(
    q_str, 10, 'c', 1, additional_read=lambda x: q_str(x, True))
types['symbol'] = TranslateType(
    str, 11, 'b', None, overwrite_read=read_symbol, overwrite_write=write_symbol)
types['month'] = TranslateType(month, 13, 'i', 4, additional_write=shandlers['month'].write(
    int), additional_read=shandlers['month'].read(month.from_int))
types['date'] = TranslateType(date, 14, 'i', 4, additional_write=shandlers['date'].write(
    write_date), additional_read=shandlers['date'].read(read_date))
types['datetime'] = TranslateType(datetime, 15, 'd', 8, additional_write=shandlers['datetime'].write(
    write_datetime), additional_read=shandlers['datetime'].read(read_datetime))
types['minute'] = TranslateType(minute, 17, 'i', 4, additional_write=shandlers[
                                'minute'].write(int), additional_read=shandlers['minute'].read(minute.from_int))
types['second'] = TranslateType(second, 18, 'i', 4, additional_write=shandlers[
                                'second'].write(int), additional_read=shandlers['second'].read(second.from_int))
types['time'] = TranslateType(time, 19, 'i', 4, additional_write=shandlers['time'].write(
    write_time), additional_read=shandlers['time'].read(read_time))
