...
class conn(object):
    ...
    SYNC = True
    ASYNC = False
    # Number of reconnect attempts to make before throwing exception
    RECONNECT_ATTEMPTS = 5
    RECONNECT_WAIT = 5000  # Milliseconds to wait between reconnect attempts
    # Maximum number of characters from query to return in exception message
    MAX_MSG_QUERY_LENGTH = 1024
    # Maximum length of a data list specified in a query before it is
    # summarized in exception message
    ...
    def connect(self, attempts=1):
        # This for loop doesn't make sense, if there's an exception it'll just
        # quit
        for attempt in range(attempts):
            try:
                self.sock.connect((self.host, self.port))
                # null terminated signed char array (bytes)
                login = array.array('b', self.user + '\0')
    ...
    def _send(self, sync, query):
        if sync:
            # 1 for synchronous requests
            message = array.array('b', [0, 1, 0, 0])
        else:
            # 1 for synchronous requests
            message = array.array('b', [0, 0, 0, 0])
        # reserve space for message length
        message.fromstring(_parser.write_integer(0))
